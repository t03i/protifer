# triton-test-stack Specification

## Purpose

The dev and integration stacks must catch the protocol- and shape-contract bugs
that a real `tritonserver` would reject in production. The previous `mock-triton`
gRPC server loaded the client's own proto, so client and mock agreed by
construction — it could not detect wrong proto field numbers, tensor rank, or
dtype, and manufactured false confidence (two such bugs shipped in two days).

This capability replaces that mock with a **real `tritonserver`** built from the
minimal `-py3-min` base plus the python backend only, small enough (~6 GB) to run
on a GitHub-hosted runner. It carries no weights: at container boot it derives a
python-backed stub model-repository from the real `model-repository/` (mounted
read-only), preserving the ensemble wire contract and replacing leaf models with
dependency-free identity models that return correctly-typed, correctly-shaped
zeros. The stub validates the protocol and shape contract — not numerical output —
and stays in lockstep with production because its repo is re-derived every boot and
its image tag is pinned equal to the production Triton tag. Worker/gateway unit
tests are out of scope: they inject a fake `TritonClient` and never spoke the wire
protocol.
